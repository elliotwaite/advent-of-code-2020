...
proc main_1 =
  ...
  for line in splitLines(input):
    let
      ...
      policy_split = line_split[0].split(' ')
      min_max_split = policy_split[0].split('-')
      min = min_max_split[0].parseInt
      max = min_max_split[1].parseInt
      char = policy_split[1][0]
      password = line_split[1]

    let char_count = password.count(char)
...
proc main_2 =
  ...
  for line in splitLines(input):
    let
      line_split = line.split(": ")
      policy_split = line_split[0].split(' ')
      pos_split = policy_split[0].split('-')
      pos_0 = pos_split[0].parseInt - 1
      pos_1 = pos_split[1].parseInt - 1
      char = policy_split[1][0]
      password = line_split[1]

    if password[pos_0] == char xor password[pos_1] == char:
      num_valid += 1
